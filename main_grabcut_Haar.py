...
#Better Bounding box using GrabCut
def find_bounding_box_new(img):
    face_cascade = cv2.CascadeClassifier(OPENCV_PATH + FACE_DETECTION_XML)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
def main(img_fg, img_bg, res_fname):
    ...
    bb_fore, bb_fore_face = find_bounding_box_new(fore_image)
    bb_back, bb_back_face = find_bounding_box_new(back_image)
