...
def find_bounding_box(img):
    ...
    print(len(rects))
    x,y,w,h = rects[-1]	
    return (x,y,w+x,h+y),(x+int(w/3.5),y+int(h/20),x+3*int(w/4.5),y+2*int(h/7.5))
...
def bounding_box_far(bb1, bb2, width):
    WIDTH_RATIO_THESHOLD = 0.1    ####decreased this
...
def prune_keypoint_matches_by_distance(matches):
    FACTOR = 8  ##decreased threshold to remove the irrelevant ones  
    pruned_matches = []
...
if __name__ == '__main__':
    ...
    fore_image = imutils.resize(fore_image, width=min(600, fore_image.shape[1]))
    back_image = imutils.resize(back_image, width=min(600, back_image.shape[1]))
     
    # color correction
    #fore_image, back_image = colorcorrect(fore_image1,back_image1)
